...
try {
    ...
    $is_paid_event = $payment_id && in_array($event_type, ['PAYMENT_RECEIVED', 'PAYMENT_CONFIRMED'], true);

    if (db_fetch('SELECT id FROM webhook_events WHERE event_id = ?', [$event_id])) {
        if ($is_paid_event) {
            if (!activate_client_subscription($payment_id, $payment)) {
                throw new Exception('Pagamento confirmado nao encontrado para ativacao: ' . $payment_id);
            }
        }
